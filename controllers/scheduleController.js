...
import { queueScheduleIngestion } from "../services/backgroundIngestionService.js"
...
export const createSchedule = asyncHandler(async (req, res) => {
  ...
  const populated = await Schedule.findById(newSchedule._id).populate("createdBy", "name email role className semester")

  queueScheduleIngestion(newSchedule._id, "create")
  console.log(`[ScheduleController] Queued ingestion for schedule: ${newSchedule._id}`)
...
export const updateSchedule = asyncHandler(async (req, res) => {
  ...
  queueScheduleIngestion(req.params.id, "update")
  console.log(`[ScheduleController] Queued ingestion for updated schedule: ${req.params.id}`)
...
export const deleteSchedule = asyncHandler(async (req, res) => {
  ...
  queueScheduleIngestion(req.params.id, "delete")
  console.log(`[ScheduleController] Queued deletion from vector DB for schedule: ${req.params.id}`)
...
export const getScheduleById = asyncHandler(async (req, res) => {
  const schedule = await Schedule.findById(req.params.id).populate("createdBy", "name email role className semester")
  if (!schedule) {
    res.status(404)
    throw new Error("Schedule not found")
  }
