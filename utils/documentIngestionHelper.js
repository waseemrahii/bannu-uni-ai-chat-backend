...
${info.lastUpdated ? `Last Updated: ${info.lastUpdated}` : ""}
  ...
  async ingestDocumentToVector(documentData) {
    try {
      const { type, sourceId, title, content, metadata } = documentData

      console.log(`[DocumentIngestion] Ingesting ${type} document: ${sourceId}`)
      ...
      const fullMetadata = {
        ...
        type,
        sourceId,
        sourceCollection: this.getCollectionName(type),
        ingestedAt: new Date(),
      }

      // Ingest to vector database
      await ragService.ingestDocument(content, fullMetadata)

      console.log(`[DocumentIngestion] Successfully ingested ${type}: ${sourceId}`)
      return { success: true, sourceId, type }
    } catch (error) {
      console.error(`[DocumentIngestion] Error ingesting document:`, error.message)
      throw error
    }
  }
...
export const ingestDocumentToVector = (documentData) => {
  return new DocumentIngestionHelper().ingestDocumentToVector(documentData)
}
